...
_checkFeatureIdx($head, undef, \%expected1Idx, 'prepended really_first, really_second');
...
sub _checkFeatureIdx {
  ...
  for my $featureName (keys %$expectedHref) {
    ...
    ok(defined $actual && $actual == $eVal,
      "Test: $testName. Can look up index of feature $featureName relative to parent " . (defined $parent ? $parent : 'header root') . " (index is $eVal)");
  }
}
