...
for my $pos (0 .. 100) {
  ...
  ok($readV->[4] eq 'world', "deleting in middle doesn\'t impact preceding adjacent value");
  ok($readV->[3] eq 'hello', "deleting in middle doesn\'t impact 2nd preceding adjacent value");
}
...
for my $pos (0 .. 100) {
  ...
  if($pos % 2) {
    ok($readV->[$dbIdx] eq "val in middle " . $pos, "could insert and read values at middle");
    next;
  }
...
for my $pos (0 .. 100) {
  ...
  ok(join(',', @{$readV->[2]}) eq join(',', 2,3), "deleting in middle doesn\'t impact preceding adjacent value");
