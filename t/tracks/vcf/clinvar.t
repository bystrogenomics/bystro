...
my $seq = MockBuilder->new_with_config({config => path('./t/tracks/vcf/clinvar.yml')->absolute, debug => 0});

my $tracks = $seq->tracksObj;
my $refBuilder = $tracks->getRefTrackBuilder();
my $refGetter = $tracks->getRefTrackGetter();

$refBuilder->db->dbPatch('chr1', $refBuilder->dbName, 1022260 - 1, $baseMapper->baseMap->{'C'}); #chr1:1022260
...
ok(@{$out} == @{$vcf->features}, "Report matching alleles, with one entry per requested feature");
...
ok($out->[$rcvIdx][0][0] eq 'RCV000116258', 'can reproduce first rcv, by splitting on pipe as defined in YAML');
ok($out->[$rcvIdx][0][1] eq 'RCV000550396', 'can reproduce second rcv, by splitting on pipe as defined in YAML');

ok($out->[$scvIdx][0][0] eq 'SCV000317028', 'can reproduce first scv, by splitting on pipe as defined in YAML');
ok($out->[$scvIdx][0][1] eq 'SCV000150176', 'can reproduce second scv, by splitting on pipe as defined in YAML');
ok($out->[$scvIdx][0][2] eq 'SCV000653894', 'can reproduce third scv, by splitting on pipe as defined in YAML');

ok(!defined $out->[$traitsIdx][0][0], 'traits are split by pipe as specified in YAML, and not_specified is cleaned to undef in the array');
ok($out->[$traitsIdx][0][1] eq 'AllHighlyPenetrant', 'traits are split by pipe as specified in YAML, finding AllHighlyPenetrant in the correct order');
ok(!defined $out->[$traitsIdx][0][2], 'traits are split by pipe as specified in YAML, and NOT_SPECIFIED is cleaned to undef in the array');
ok($out->[$traitsIdx][0][3] eq 'Myasthenic_syndrome..congenital..8', 'traits are split by pipe as specified in YAML, finding Myasthenic_syndrome..congenital..8 in the correct order');
