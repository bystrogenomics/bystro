...
for my $pos (0 .. $#$mainDbAref) {
  ...
  if($pos >= 2200 && $pos < 2300) {
    ok($dist == 0);
    ok(join(",", sort {$a cmp $b} @$name) eq join(",", sort {$a cmp $b} 'NR_137295','NR_FAKE1','NR_FAKE2'));
    ok(join(",", sort {$a cmp $b} @$name2) eq join(',', sort {$a cmp $b} 'RNR2','RNR2_FAKE1','RNR2_FAKE2'));

    # same as nGene within the gene when storeNearest is false
    ok(join(",", sort {$a cmp $b} @$name2Gene) eq join(",", sort {$a cmp $b} 'RNR2','RNR2_FAKE1','RNR2_FAKE2'));
    # ok(!defined $distGene);
  ...
  if($pos >= 2300 && $pos < 3230) {
    ok($dist == 0);
    ok(join(",", sort {$a cmp $b } @$name) eq join(",", sort {$a cmp $b} 'NR_137295','NR_FAKE2'));
    ok(join(",", sort { $a cmp $b } @$name2) eq join(',', sort {$a cmp $b} 'RNR2', 'RNR2_FAKE2'));

    # We don't guarantee transcript order atm, but all features will be correct relative to
    # all transcripts
    ok(join(",", sort {$a cmp $b} @$name2Gene) eq join(",", sort {$a cmp $b} 'RNR2','RNR2_FAKE2'));
    # ok(!defined $distGene);
    ...
    if($pos < 2200 + (3800 - 2200) / 2) {
      ...
      #intergenic (or on top of @2200) and past/on top of the last transcript (NR_FAKE2)
      ok($distTss == 3800 - $pos);
      ok(join(';', sort { $a cmp $b } @$nameTss) eq join(';', sort { $a cmp $b }'NR_FAKE3', 'NR_FAKE3B', 'NR_FAKE3C'));
  ...
  if($pos >= 3800 && $pos < 5000) {
    if($pos < 4000) {
      ok($dist == 0);
      ok(join(",", sort {$a cmp $b} @$name) eq join(",", sort {$a cmp $b} 'NR_FAKE3', 'NR_FAKE3B', 'NR_FAKE3C'));
      ok(join(",", @$name2) eq 'FAKE3');
      ...
      ok($distTss == 3800 - $pos);
      ok(join(",", sort {$a cmp $b} @$nameTss) eq join(",", sort {$a cmp $b} @$name));
      ok(join(",", @$name2Tss) eq join(",", @$name2));
    }

    if($pos >= 4000 && $pos < 4100) {
      ok($dist == 0);
      ok(join(",", sort {$a cmp $b} @$name) eq join(",", sort {$a cmp $b} 'NR_FAKE3B', 'NR_FAKE3C'));
    ...
    if($pos >= 4100) {
      ...
      if($pos >= 4300 && $pos < 4500) {
        ok($dist == 0);
        ok(join(",", sort {$a cmp $b} @$name) eq join(",", sort {$a cmp $b} 'NR_FAKE3C', 'NR_FAKE4'));
        ok(join(",", sort {$a cmp $b} @$name2) eq join(",", sort {$a cmp $b} 'FAKE3', 'FAKE4'));

        # ok($distGene == 0);
        ok(join(",", sort {$a cmp $b} @$name2Gene) eq join(",", sort {$a cmp $b} @$name2));
      }
