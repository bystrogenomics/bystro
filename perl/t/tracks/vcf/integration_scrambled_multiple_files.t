...
my $config = './t/tracks/vcf/test.scrambled_multiple_files.yml';

my $file = LoadFile($config);
my $dbPath = $file->{database_dir};
path($dbPath)->remove_tree({keep_root => 1});

my $seq = MockBuilder->new_with_config({config => path($config)->absolute, debug => 0});
...
for my $feature (@{$vcf->features}) {
  ...
  ok(@{$out->[$idx]} == 1, "Every feature is considered bi-allelelic (because alleles are decomposed into bi-allelic sites, with 1 entry");
  ok(!ref $out->[$idx][0], "Every feature contains a single position's worth of data, and that value is scalar");
}

$out = [];
$vcf->get($href, 'chr22', 'C', '+ACA', 1, $out);
ok(@$out == 0, "Vcf getter does not tile annotations by the position of the allele. Only accepts position index 0");
...
ok($out->[$afIdx][0] == unpack('f', pack('f', 1.81378e-04)), "correctly finds first the +ACA allele af value");
ok($out->[$anIdx][0] == 16540, "correctly finds first the +ACA allele an value, which has only one value across all alleles");

ok($out->[$acAfrIdx][0] == 0, "correctly finds first the +ACA allele ac_afr value");
ok($out->[$acAmrIdx][0] == 0, "correctly finds first the +ACA allele ac_amr value");
ok($out->[$acAsjIdx][0] == 0, "correctly finds first the +ACA allele ac_asj value");
ok($out->[$acEasIdx][0] == 0, "correctly finds first the +ACA allele ac_eas value");
ok($out->[$acFinIdx][0] == 0, "correctly finds first the +ACA allele ac_fin value");
ok($out->[$acNfeIdx][0] == 3, "correctly finds first the +ACA allele ac_nfe value");
ok($out->[$acOthIdx][0] == 0, "correctly finds first the +ACA allele ac_oth value");
ok($out->[$acMaleIdx][0] == 3, "correctly finds first the +ACA allele ac_male value");
ok($out->[$acFemaleIdx][0] == 0, "correctly finds first the +ACA allele ac_female value");

ok($out->[$anAfrIdx][0] == 3614, "correctly finds first the +ACA allele an_afr value, which has only one value across all alleles");
ok($out->[$anAmrIdx][0] == 528, "correctly finds first the +ACA allele an_amr value, which has only one value across all alleles");
ok($out->[$anAsjIdx][0] == 180, "correctly finds first the +ACA allele an_asj value, which has only one value across all alleles");
ok($out->[$anEasIdx][0] == 892, "correctly finds first the +ACA allele an_eas value, which has only one value across all alleles");
ok($out->[$anFinIdx][0] == 2256, "correctly finds first the +ACA allele an_fin value, which has only one value across all alleles");
ok($out->[$anNfeIdx][0] == 8466, "correctly finds first the +ACA allele an_nfe value, which has only one value across all alleles");
...
ok($out->[$afEasIdx][0] == 0, "correctly finds first the +ACA allele af_eas value");
ok($out->[$afFinIdx][0] == 0, "correctly finds first the +ACA allele af_fin value");
ok($out->[$afNfeIdx][0] == unpack('f', pack('f', 3.54359e-04)), "correctly finds first the +ACA allele af_nfe value");
ok($out->[$afOthIdx][0] == 0, "correctly finds first the +ACA allele af_oth value");
ok($out->[$afMaleIdx][0] == unpack('f', pack('f', 3.22303e-04)), "correctly finds first the +ACA allele af_male value");
ok($out->[$afFemaleIdx][0] == 0, "correctly finds first the +ACA allele af_female value");
...
ok($out->[$afIdx][0] == unpack('f', pack('f', 6.93049e-05)), "correctly finds the af value for a biallelic site");
ok($out->[$anIdx][0] == 28858, "correctly finds the an value for a biallelic site");
...
ok($out->[$acFemaleIdx][0] == 1, "correctly finds C allele allele ac_female value");


ok($out->[$anAfrIdx][0] == 8454, "correctly finds the C allele an_afr value, which has only one value across all alleles");
ok($out->[$anAmrIdx][0] == 782, "correctly finds the C allele an_amr value, which has only one value across all alleles");
ok($out->[$anAsjIdx][0] == 232, "correctly finds the C allele an_asj value, which has only one value across all alleles");
ok($out->[$anEasIdx][0] == 1606, "correctly finds the C allele an_eas value, which has only one value across all alleles");
ok($out->[$anFinIdx][0] == 3132, "correctly finds the C allele an_fin value, which has only one value across all alleles");
ok($out->[$anNfeIdx][0] == 13774, "correctly finds the C allele an_nfe value, which has only one value across all alleles");
ok($out->[$anOthIdx][0] == 878, "correctly finds the C allele an_oth value, which has only one value across all alleles");
ok($out->[$anMaleIdx][0] == 15900, "correctly finds the C allele an_male value, which has only one value across all alleles");
ok($out->[$anFemaleIdx][0] == 12958, "correctly finds the C allele an_female value, which has only one value across all alleles");
...
ok($out->[$afNfeIdx][0] == unpack('f', pack('f', 1.45201e-04)), "correctly finds the C allele af_nfe value");
ok($out->[$afOthIdx][0] == 0, "correctly finds the C allele af_oth value");
ok($out->[$afMaleIdx][0] == unpack('f', pack('f', 6.28931e-05)), "correctly finds the C allele af_male value");
ok($out->[$afFemaleIdx][0] == unpack('f', pack('f', 7.71724e-05)), "correctly finds the C allele af_female value");
...
ok($firstAllele->[$trTvIdx][0] == 2, "trTv is 0 for multiallelics.
...
ok(!defined $firstAllele->[$idIdx][0], "correctly finds that this site has no rsID");
ok($firstAllele->[$acIdx][0] == 1, "correctly finds the ac value for a biallelic site");
ok($firstAllele->[$afIdx][0] == unpack('f', pack('f', 4.21905e-05)), "correctly finds the af value for a biallelic site");
ok($firstAllele->[$anIdx][0] == 23702, "correctly finds the an value for a biallelic site");

ok($firstAllele->[$acAfrIdx][0] == 1, "correctly finds the T allele ac_afr value");
ok($firstAllele->[$acAmrIdx][0] == 0, "correctly finds T allele allele ac_amr value");
ok($firstAllele->[$acAsjIdx][0] == 0, "correctly finds T allele allele ac_asj value");
ok($firstAllele->[$acEasIdx][0] == 0, "correctly finds T allele allele ac_eas value");
ok($firstAllele->[$acFinIdx][0] == 0, "correctly finds T allele allele ac_fin value");
ok($firstAllele->[$acNfeIdx][0] == 0, "correctly finds T allele allele ac_nfe value");
ok($firstAllele->[$acOthIdx][0] == 0, "correctly finds T allele allele ac_oth value");
ok($firstAllele->[$acMaleIdx][0] == 1, "correctly finds T allele allele ac_male value");
ok($firstAllele->[$acFemaleIdx][0] == 0, "correctly finds T allele allele ac_female value");


ok($firstAllele->[$anAfrIdx][0] == 6452, "correctly finds the T allele an_afr value, which has only one value across all alleles");
ok($firstAllele->[$anAmrIdx][0] == 638, "correctly finds the T allele an_amr value, which has only one value across all alleles");
ok($firstAllele->[$anAsjIdx][0] == 222, "correctly finds the T allele an_asj value, which has only one value across all alleles");
ok($firstAllele->[$anEasIdx][0] == 1398, "correctly finds the T allele an_eas value, which has only one value across all alleles");
ok($firstAllele->[$anFinIdx][0] == 2270, "correctly finds the T allele an_fin value, which has only one value across all alleles");
ok($firstAllele->[$anNfeIdx][0] == 12010, "correctly finds the T allele an_nfe value, which has only one value across all alleles");
ok($firstAllele->[$anOthIdx][0] == 712, "correctly finds the T allele an_oth value, which has only one value across all alleles");
ok($firstAllele->[$anMaleIdx][0] == 13204, "correctly finds the T allele an_male value, which has only one value across all alleles");
ok($firstAllele->[$anFemaleIdx][0] == 10498, "correctly finds the T allele an_female value, which has only one value across all alleles");

ok($firstAllele->[$afAfrIdx][0] == unpack('f', pack('f', 1.54991e-04)), "correctly finds the T allele af_afr value");
...
ok($firstAllele->[$afMaleIdx][0] == unpack('f', pack('f', 7.57346e-05)), "correctly finds the T allele af_male value");
ok($firstAllele->[$afFemaleIdx][0] == 0, "correctly finds the T allele af_female value");
...
ok($secondAllele->[$trTvIdx][0] == 2, "trTv is 0 for multiallelics. \
  However, when our vcf parser is passed 2 alleles at the same position, but on different source file lines, it treats that as a SNP.
  Therefore it calls it as a G->C transversion, or 2");
ok($secondAllele->[$idIdx][0] eq 'rs199856444', "correctly finds that this site has an rsID");
ok($secondAllele->[$acIdx][0] == 5232, "correctly finds the ac value for a biallelic site");
ok($secondAllele->[$afIdx][0] == unpack('f', pack('f', 2.00721e-01)), "correctly finds the af value for a biallelic site");
ok($secondAllele->[$anIdx][0] == 26066, "correctly finds the an value for a biallelic site");

ok($secondAllele->[$acAfrIdx][0] == 1195, "correctly finds the C allele ac_afr value");
ok($secondAllele->[$acAmrIdx][0] == 199, "correctly finds C allele allele ac_amr value");
ok($secondAllele->[$acAsjIdx][0] == 48, "correctly finds C allele allele ac_asj value");
ok($secondAllele->[$acEasIdx][0] == 462, "correctly finds C allele allele ac_eas value");
ok($secondAllele->[$acFinIdx][0] == 539, "correctly finds C allele allele ac_fin value");
ok($secondAllele->[$acNfeIdx][0] == 2634, "correctly finds C allele allele ac_nfe value");
ok($secondAllele->[$acOthIdx][0] == 155, "correctly finds C allele allele ac_oth value");
ok($secondAllele->[$acMaleIdx][0] == 2860, "correctly finds C allele allele ac_male value");
ok($secondAllele->[$acFemaleIdx][0] == 2372, "correctly finds C allele allele ac_female value");

ok($secondAllele->[$anAfrIdx][0] == 7838, "correctly finds the C allele an_afr value, which has only one value across all alleles");
ok($secondAllele->[$anAmrIdx][0] == 630, "correctly finds the C allele an_amr value, which has only one value across all alleles");
ok($secondAllele->[$anAsjIdx][0] == 216, "correctly finds the C allele an_asj value, which has only one value across all alleles");
ok($secondAllele->[$anEasIdx][0] == 1372, "correctly finds the C allele an_eas value, which has only one value across all alleles");
ok($secondAllele->[$anFinIdx][0] == 2596, "correctly finds the C allele an_fin value, which has only one value across all alleles");
ok($secondAllele->[$anNfeIdx][0] == 12638, "correctly finds the C allele an_nfe value, which has only one value across all alleles");
ok($secondAllele->[$anOthIdx][0] == 776, "correctly finds the C allele an_oth value, which has only one value across all alleles");
ok($secondAllele->[$anMaleIdx][0] == 14358, "correctly finds the C allele an_male value, which has only one value across all alleles");
ok($secondAllele->[$anFemaleIdx][0] == 11708, "correctly finds the C allele an_female value, which has only one value across all alleles");

ok($secondAllele->[$afAfrIdx][0] == unpack('f', pack('f', 1.52462e-01)), "correctly finds the C allele af_afr value");
ok($secondAllele->[$afAmrIdx][0] == unpack('f', pack('f', 3.15873e-01)), "correctly finds the C allele af_amr value");
ok($secondAllele->[$afAsjIdx][0] == unpack('f', pack('f', 2.22222e-01)), "correctly finds the C allele af_asj value");
ok($secondAllele->[$afEasIdx][0] == unpack('f', pack('f', 3.36735e-01)), "correctly finds the C allele af_eas value");
ok($secondAllele->[$afFinIdx][0] == unpack('f', pack('f', 2.07627e-01)), "correctly finds the C allele af_fin value");
ok($secondAllele->[$afNfeIdx][0] == unpack('f', pack('f', 2.08419e-01)), "correctly finds the C allele af_nfe value");
ok($secondAllele->[$afOthIdx][0] == unpack('f', pack('f', 1.99742e-01)), "correctly finds the C allele af_oth value");
  ...
  for my $feature (@{$vcf->features}) {
    ...
    if($alleleIdx == 0) {
      if(!defined $multiallelic->[$alleleIdx][$featureIdx][$posIdx]) {
        ok(!defined $firstAllele->[$alleleIdx][$featureIdx][$posIdx],
        ...
        ok($multiallelic->[$alleleIdx][$featureIdx][$posIdx] == $firstAllele->[$featureIdx][$posIdx],
          "multiallelics are reproduced just like bi-allelics, but on single line for feature " . $vcf->features->[$featureIdx]);
      } else {
        ok($multiallelic->[$alleleIdx][$featureIdx][$posIdx] eq $firstAllele->[$featureIdx][$posIdx],
          "multiallelics are reproduced just like bi-allelics, but on single line for feature " . $vcf->features->[$featureIdx]);
      }
    ...
    if($alleleIdx == 1) {
      if(!defined $multiallelic->[$alleleIdx][$featureIdx][$posIdx]) {
        ok(!defined $secondAllele->[$featureIdx][$posIdx],
          "multiallelics are reproduced just like bi-allelics, but on single line for feature " . $vcf->features->[$featureIdx]);
      } elsif(looks_like_number($multiallelic->[$alleleIdx][$featureIdx][$posIdx])) {
        ok($multiallelic->[$alleleIdx][$featureIdx][$posIdx] == $secondAllele->[$featureIdx][$posIdx],
          "multiallelics are reproduced just like bi-allelics, but on single line for feature " . $vcf->features->[$featureIdx]);
      } else {
        ok($multiallelic->[$alleleIdx][$featureIdx][$posIdx] eq $secondAllele->[$featureIdx][$posIdx],
          "multiallelics are reproduced just like bi-allelics, but on single line for feature " . $vcf->features->[$featureIdx]);
      }
    }
  }
