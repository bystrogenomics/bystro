...
ok("$floatConvert" eq "1.567e-13", "perl represents large floats in scientific notation");
...
ok(looks_like_number($floatConvert), "number looks like number");
ok(looks_like_number($roundedConvert), "number(2) looks like number");
ok($floatConvert == 1, "number converts floats to ints when exact solution possible");
ok($roundedConvert == 1, "number converts floats to ints when exact solution possible");
...
ok(looks_like_number($floatConvert), "float looks like number");
ok(looks_like_number($roundedConvert), "number looks like number");
ok($floatConvert == 123000000.259,"number Can convert number larger than 1 million to float");
...
ok($floatConvert == .0000000000001, "number can convert scientific notation string");
ok($roundedConvert == .0000000000001, "number(2) can convert scientific notation string; will not round if fewer sigfigs available than specified");
