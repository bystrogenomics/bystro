...
ok(
  length($packed) == 1,
  "The string $numThatShouldBeInt takes 1 byte in msgpack, when using the 'number' converter"
);
...
ok(
  length($packed) == 5,
  "With prefer_float32 floating point numbers will be packed in 5 bytes/single precision"
);

$packed = $mp->pack( "1.000000" +0 );
ok( length($packed) == 5,
  "The string '1.000000' + 0 takes 5bytes in msgpack with prefer_float32" );
