...
sub BUILDARGS {
  ...
  if ( defined $data->{outputExtensions} ) {
    if (
      !(
           $data->{outputExtensions}{json}
        && $data->{outputExtensions}{qc}
        && $data->{outputExtensions}{tab}
      )
      )
    {
...
sub getStatsArguments {
  ...
  if (
    !$statsProg
    && ( $snpNameColumnName
      && $exonicAlleleFuncColumnName
      && $emptyFieldString
      && $valueDelimiter
      && $refColumnName
      && $alleleColumnName
      && $siteTypeColumnName
      && $homozygotesColumnName
      && $heterozygotesColumnName
      && $jsonOutPath
      && $tabOutPath
      && $qcOutPath )
    )
  {
    return (
      "Need, refColumnName, alleleColumnName, siteTypeColumnName, homozygotesColumnName,"
        . "heterozygotesColumnName, jsonOutPath, tabOutPath, qcOutPath, "
        . "primaryDelimiter, fieldSeparator, and "
        . "numberHeaderLines must equal 1 for statistics",
      undef, undef
    );
  }
  ...
  return ( undef,
        "$statsProg -outJsonPath $jsonOutPath -outTabPath $tabOutPath "
