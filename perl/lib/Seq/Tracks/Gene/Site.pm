...
sub pack {
  my ( $self, $txNumber, $siteType, $strand, $codonNumber, $codonPosition, $codonSeq )
    = @_;
  ...
  if ( defined $codonNumber || defined $codonPosition || defined $codonSeq ) {
    ...
    if ( !( looks_like_number($codonPosition) && looks_like_number($codonNumber) ) ) {
      $self->log( 'fatal',
        "codonPosition && codonNumber must be numeric, got $codonPosition && $codonNumber" );
    }
...
sub unpack {
  ...
  if ( !ref $_[1]->[0] ) {
    ...
    return (
      $_[1]->[0],
      [
        ( @{ $combinedMap->{ $_[1]->[1] } } ), $_[1]->[2],
        $_[1]->[3],                            $codonMap->num2Codon( $_[1]->[4] )
      ]
    );
  }
