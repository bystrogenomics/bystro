...
state $siteTypes = [
  $codingSite,    $fivePrimeSite, $threePrimeSite, $spliceAcSite,
  $spliceDonSite, $ncRNAsite,     $intronicSite
];
...
has siteTypeMap => (
  ...
  isa      => 'HashRef',
  traits   => ['Hash'],
  handles  => { getSiteTypeFromNum => 'get', },
  lazy     => 1,
  init_arg => undef,
...
has exonicSites => (
  ...
  isa      => 'HashRef',
  traits   => ['Hash'],
  handles  => { isExonicSite => 'exists', },
  default  => sub {
    return { map { $_ => 1 }
        ( $codingSite, $ncRNAsite, $fivePrimeSite, $threePrimeSite ) };
  },
);
