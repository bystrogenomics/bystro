...
sub go {
  ...
  $pm->run_on_finish(sub{
    ...
    if($exitCode != 0) {
      $self->log('fatal', "$liftedPath failed liftOver due to: " . ($outOrErrRef ? $$outOrErrRef : 'unknown error'));
      return;
    }
      ...
      while(my $l = $readFh->getline() ) {
        ...
        if(!$outFh && !$dryRun) {
          ...
          if(-e $outPath && !$self->overwrite) {
            $self->log('warn', "outPath $outPath exists, skipping $inPath because overwrite not set");
            last;
          }
        ...
        if(defined $caddDbVals && @$caddDbVals != 3) {
          ...
          my $err = "Couldn't find 3 cadd values for $chr\:$pos ($based\-based) ... Found " . (scalar @$caddDbVals) . " instead";
          $self->log('error', \$err);
