...
class BinomialMafFilter(Struct, frozen=True, tag='binomMaf', tag_field='key', forbid_unknown_fields=True):
    """
    Example: {'critValue': '.05', 'estimates': ['gnomad.genomes.af', 'gnomad.exomes.af'], 'privateMaf': 0.0001, 'snpOnly': True, 'numSamples': 3, 'key': 'binomialMaf'}
    """
    privateMaf: float
    snpOnly: bool
    numSamples: int
    estimates: list[str]
    critValue: float | None = .05

class HWEFilter(Struct, frozen=True, tag='hwe', tag_field='key', forbid_unknown_fields=True):
    """
