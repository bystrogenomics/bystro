...
class MVTadaZipEM(BaseSGDModel):
    ...
    def fit(self, data, progress_bar=True):
        ...
        for i in myrange(training_options["n_iterations"]):
            ...
            for k in range(K):
                ...
                log_likelihood_0_b = torch.broadcast_to(log_likelihood_0, X.shape)
    ...
    def predict_logproba(self, data):
        ...
        for i in range(N):
            for k in range(self.K):
                ...
                log_prob_0 = np.log(
                    self.Alpha[k] + (1 - self.Alpha[k]) * np.exp(-1 * self.Lambda[k])
                )
    ...
    def _initialize_variables(self, X):
        ...
        alpha_ls = [torch.tensor(pct_0_inv, requires_grad=True) for i in range(self.K)]
