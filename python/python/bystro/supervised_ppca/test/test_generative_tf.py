...
from ..gf_generative_tf import PPCAtf, SPCAtf, FactorAnalysistf
...
def test_spca():
    ...
    idxs = np.zeros(35)
    idxs[30:] = 1
    model.fit(XX, groups=idxs)

    cov_est = model.get_covariance()
    cov_emp = np.dot(XX.T, XX) / XX.shape[0]
    s1 = la.norm(cov_emp - cov_est)
    assert s1 <= 10.0
...
def test_factoranalysis():
    L = 4
    X, S_factor, W_, phi = generate_data_factorAnalysis()
    model = FactorAnalysistf(L)
    model.fit(X)
    cov_est = model.get_covariance()
    cov_emp = np.dot(X.T, X) / X.shape[0]
    s1 = la.norm(cov_emp - cov_est)
