...
def _coerce_inputs(job_details, job_id, publisher_host, publisher_port, progress_event, event_queue, config_path_base_dir):
    job_specific_args = {}
    ...
    config_file_path = _get_config_file_path(config_path_base_dir, job_specific_args['assembly'])

    if not config_file_path:
        raise Exception(f"Assembly {job_specific_args['assembly']} doesn't have corresponding config file")
...
def listen(queue_conf: dict, search_conf: dict, config_path_base_dir: str):
    ...
    clients = tuple((h, ports[i], BeanstalkClient(h, ports[i], socket_timeout=10)) for i,h in enumerate(hosts))
    ...
    while True:
        ...
        try:
            ...
            print(f"Reserved job with id {job.job_id} which contains:")

            job_data: dict = loads(job.job_data)
            pp(job_data)

            # create the annotator
            input: dict = _coerce_inputs(job_data, job.job_id,
                                        publisher_host = host,
                                        publisher_port = port,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--queue_conf', type=str, help='Path to the beanstalkd queue config yaml file (e.g beanstalk1.yml)')
    parser.add_argument('--search_conf', type=str, help='Path to the opensearch config yaml file (e.g. elasticsearch.yml)')
